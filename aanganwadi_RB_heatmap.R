...
ui <- bootstrapPage(
  ...
  absolutePanel(
    ...
    selectInput(
      "KGISTalukName", "Select the Taluka Name:",
...
server <- function(input, output, session) {
  output$mymap <- renderLeaflet({
    filtered_data <- bqdata %>%
            dplyr::filter(if (input$KGISVillageName == "All") {
                KGISVillageName != ""
            } else {
                KGISVillageName == input$KGISVillageName
            }) %>%
            dplyr::filter(if (input$KGISTalukName == "All") {
                KGISTalukName != ""
            } else {
                KGISTalukName == input$KGISTalukName
            })
